...
class TriageService {
  ...
  private async analyzeFinding(finding: any, config: TriageConfig): Promise<any> {
    ...
    const thresholdOrder = severityOrder[config.severityThreshold as keyof typeof severityOrder] || 0;
    const findingOrder = severityOrder[analysis.triagedSeverity as keyof typeof severityOrder] || 0;
