      ...
      for (const finding of findingsData) {
        try {
          ...
          if (config.autoSubmit && config.platform !== 'custom') {
            ...
            // Save as draft
            const draftReport = await this.saveDraftReport(report);
  ...
  private async submitToPlatform(report: any, config: ReportConfig): Promise<any> {
    ...
    try {
      ...
      switch (config.platform) {
        case 'hackerone':
          response = await this.submitToHackerOne(report, apiKey);
          break;
        case 'bugcrowd':
          response = await this.submitToBugcrowd(report, apiKey);
          break;
        case 'yeswehack':
          response = await this.submitToYesWeHack(report, apiKey);
          break;
        case 'intigriti':
          response = await this.submitToIntigriti(report, apiKey);
          break;
        default:
