...
import { reconService, ReconConfig } from './reconService.js';
import { scanningService, ScanConfig } from './scanningService.js';
import { exploitationService, ExploitationConfig } from './exploitationService.js';
import { triageService, TriageConfig } from './triageService.js';
import { reportingService, ReportConfig } from './reportingService.js';
...
reconQueue.process(async (job) => {
  const data = job.data as { jobId: string; config: ReconConfig };
...
scanningQueue.process(async (job) => {
  const data = job.data as { jobId: string; config: ScanConfig };
  const jobId = data.jobId;
  const config = data.config;
...
exploitationQueue.process(async (job) => {
  const data = job.data as { jobId: string; config: ExploitationConfig };
  const jobId = data.jobId;
  const config = data.config;
